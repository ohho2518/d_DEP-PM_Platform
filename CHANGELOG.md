...
- **Fix ระหว่างทาง:** `POST /api/ceo/report/:id` ไม่เคย `commit()` → audit `ceo.reported`
  ถูกทิ้งทุกครั้ง (เขียนแล้วหายไปพร้อม session) — เพิ่ม commit ทั้งเส้นทาง endpoint และอัตโนมัติ
