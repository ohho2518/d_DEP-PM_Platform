...
## 2026-08-28 — 🔴 scaffold เขียนทับพื้นที่งานไม่ได้อีกแล้ว (บั๊กที่ทำของหายจริง 2 ครั้ง)

**เกิดอะไรขึ้น:** `POST /bootstrap` ที่มี `target` = `D:\Dev_Proj` (รากพอดี) เอาแม่แบบ kit
ไปทับ `AGENTS.md` · `PROJECT_STATUS.md` · `CHANGELOG.md` · `docs/` ของ**พื้นที่งานทั้งก้อน**
— ครั้งแรก 26 ส.ค. (ชื่อ `d_ShopeeFeed`) ครั้งที่สอง 28 ส.ค. (`d_Songtify`) ·
กู้ไม่ได้เพราะ repo ที่รากไม่มี commit เลย และไม่มี shadow copy

**ต้นเหตุ:** `resolve_target()` เขียนเงื่อนไขว่า `if root not in p.parents and p != root: raise`
— ท่อน `p != root` แปลว่า **"รากพอดีก็ผ่าน"** · ฟอร์มเก่าใส่ค่าปริยาย `D:\Dev_Proj\`
ไว้ในช่อง target อยู่แล้ว ⇒ กดสร้างโดยไม่แก้ช่องนั้น = ยิงลงรากทันที

**ปิดไป 3 ชั้น**

1. **`resolve_target` ปฏิเสธ target ที่เท่ากับราก** (400 พร้อมบอกว่าต้องใส่โฟลเดอร์ของโปรเจกต์เอง)
2. **`backup_before_overwrite()`** — ปลายทางมีไฟล์เดิมที่กำลังจะถูกทับ ต้องได้สำเนาลง
   `<target>/BackUp/Scaffold_<เวลา>/` ก่อนเสมอ แล้วบอกใน `steps[]` (WORKING_RULES Rule 1)
   · รายชื่อไฟล์คิดจากของจริงใน kit ไม่ใช่ลิสต์ที่พิมพ์ค้างไว้
3. **UI ประกอบ path ให้เอง** (ราก + ทีม + ชื่อโปรเจกต์) — ไม่มีช่องที่ค่าปริยายเป็นรากอีกแล้ว
- 🐛 **แถม:** เรียก git ด้วย `text=True` เปล่า ๆ ทำให้ Windows ถอดรหัส output ด้วย ANSI
  ของเครื่อง (cp874) แล้ว**พังทันทีที่ชื่อโฟลเดอร์เป็นภาษาไทย** — บังคับ UTF-8 แล้ว
  (เจอเพราะเทสต์ใหม่ตั้งชื่อโฟลเดอร์เป็นไทย · ตรงกับ WORKING_RULES §6.1 เป๊ะ)
- กติกาใหม่ **§9.1.17** · pytest 257 → **260**

## 2026-08-28 — ฟอร์มเปิดโปรเจกต์ใหม่กลับมาทำงานเหมือน `new-project-studio`

ผู้ใช้ทักว่า "การทำงานไม่เหมือนเดิม" เทียบกับ `D:\Dev_Proj\4_RND\new-project-studio` —
ตรวจแล้วพบว่าตอนยก ADR-05 เข้ามา **`services/scaffold.py` ยกมาครบทุกบรรทัด แต่ฟอร์มบนหน้าเว็บ
ยกมาไม่ครบ** ฝั่ง backend รับ `team`/`dual_ps` อยู่แล้วแต่หน้าเว็บไม่เคยส่งให้

- **เลือกทีมเจ้าของแทนพิมพ์ path เอง** — `GET /api/projects/scaffold-options` คืนราก +
  โฟลเดอร์ทีมที่มีอยู่จริง (อ่านจากดิสก์ ไม่ใช่รายชื่อตายตัว) แล้วฟอร์มประกอบ
  `<ราก>\<ทีม>\<ชื่อ>` ให้เอง · ค่าเริ่มต้น `_INBOX` · พิมพ์ทับเองได้ (เลือกทีมใหม่ = กลับมา auto)
  · ราก **เลิก hardcode `D:\Dev_Proj` ฝั่ง frontend** — มาจาก `SCAFFOLD_ALLOWED_ROOT` เท่านั้น
- **ช่อง "สังกัดทีมไหนใน ecosystem"** (เมื่อเลือก `eco-team`) และ **ติ๊ก "เป็น Product/Service ด้วย"**
  (เมื่อเลือก `eco-team`/`middleware`) กลับมาแล้ว — ไปต่อท้ายบรรทัด Ecosystem ใน `AGENTS.md`
- **ลากไฟล์ดีไซน์มาแนบตอนเปิดโปรเจกต์ได้** — เก็บลง `_design_input/` ทันทีหลังสร้างโฟลเดอร์
  · แนบไม่สำเร็จ **ไม่กลบ**ความจริงว่าโฟลเดอร์ถูกสร้างแล้ว (บอกให้ไปแนบซ้ำที่บอร์ด)
- **ปุ่ม "📌 Commit เลย"** — `POST /api/projects/:id/commit` (`scaffold.git_commit_initial()`
  มีมาตั้งแต่ ADR-05 แต่ไม่เคยมีทางกดจากหน้าเว็บ) · ยัง**ไม่ auto-commit** เหมือนเดิม
- **ชนิดงานที่เลือกไม่ถูกทิ้งอีกแล้ว** — `/bootstrap` รับ `kind` (`code` ปริยาย · `doc`) แล้ว
  เลือก "งานเอกสาร" จึงได้เส้นทางของงานเอกสารจริง (เดิมได้ `code` เสมอ เพราะฟอร์มไม่เคยส่งค่านี้)
  · `kind: "idea"` = **422** — ไอเดียคือสิ่งที่ยังไม่ลงมือ ไม่ควรมีโฟลเดอร์จริง (ต้องผ่าน `/promote`)
- **สิ่งที่ตั้งใจไม่ยกกลับมา:** `ai_fill` ที่เรียก `claude -p` CLI (ขัด §9.1.13 — ทุก LLM call
  ผ่าน `providers.py`) · ตัวปิด server หลัง commit (ที่นี่เป็นแพลตฟอร์มที่รันค้าง ไม่ใช่เครื่องมือใช้ครั้งเดียว)
  · terminal สตรีมสด — ขั้นตอนทั้งหมดยังโชว์ครบใน `steps[]` หลังทำเสร็จ
- pytest 250 → **257** · ruff clean · `npm run build` ผ่าน
