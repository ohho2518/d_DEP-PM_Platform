...
>
> ### 1. 🔑 คีย์ Gemini ใน `backend/.env` ใช้ไม่ได้ (ตรวจจริง 2026-08-14)
> ตอบ **401 UNAUTHENTICATED — "Expected OAuth 2 access token"** ⇒ ค่าที่ใส่ไว้เป็น OAuth token
> ไม่ใช่ API key ของ Google AI Studio · ต้องออกคีย์ใหม่แล้วกรอกที่หน้า **`/settings`**
> (Anthropic ✅ · OpenAI ✅ — ยิงจริงผ่านทั้งคู่)
>
...
- **ผลข้างเคียงที่มีค่า:** ✅ **OpenAI `gpt-5.2` ใช้ได้จริง** (ปิด Risk #5 ที่ค้างมาตั้งแต่ Sprint 4)
  · ❌ **คีย์ Gemini ใน `.env` ใช้ไม่ได้** (401 — เป็น OAuth token ไม่ใช่ API key)
...
- **ผู้ให้บริการ AI:** Anthropic ✅ · OpenAI ✅ (ยืนยันด้วยการยิงจริง 2026-08-14) ·
  Gemini ❌ 401 (คีย์ผิดชนิด) · แก้/ทดสอบได้เองที่หน้า `/settings` โดยไม่ต้อง restart
...
- **ของทดสอบที่ค้างในระบบ (เก็บไว้เป็นหลักฐาน):** d_CEO `4eb918bd` = `awaiting_approval`
  รอ Vinit · DEP-PM `c8d5c50c` (รอบ 3) · `bc3a43b7` · `17db2a67` (needs_human) ·
  `4acff915` (failover สำเร็จ — มีป้ายตัวสำรองในผลงาน) + อีก 2 ตัวจากทดสอบ degrade
- git: main สะอาด ณ ต้นรอบ (`2b9bf39`, push แล้ว) · **งานรอบนี้ยังไม่ commit** (รอผู้ใช้ตรวจ)

## Next Tasks

1. 🔑 **ออกคีย์ Gemini ใหม่** (Google AI Studio) แล้วกรอกที่ `/settings` → กดทดสอบให้ขึ้นเขียว
   — ตัวที่อยู่ใน `.env` ตอนนี้เป็น OAuth token ใช้กับ API ไม่ได้ · **ต้องให้เจ้าของทำ**
1.1 🔴 **แยกคีย์ต่อรีโปที่คอนโซลผู้ให้บริการ** — ตราบใดที่ยังใช้คีย์ร่วมกัน บัญชีหมดเครดิต
...
5. **Team Mode กับคีย์จริง** — ตอนนี้พิสูจน์แล้วว่า OpenAI ใช้ได้ ⇒ ลอง `AGENT_MODE=team`
   1 โปรเจกต์เพื่อวัด token accounting ของ OpenAI กับงานจริง (Gemini รอคีย์ใหม่)
6. **§5 ของใบสั่งงาน — เพดานค่าใช้จ่ายแยกต่อผู้ให้บริการ:** ยังไม่มีใครในบ้านทำ ·
   ต้องนับโทเคนต่อเจ้าก่อนถึงจะมีถังให้แยก (ตอนนี้ `LLMReply` มี `provider` แล้ว = มีที่เกาะ)
...
- 🔑 **คีย์ Gemini ใน `.env` ใช้ไม่ได้** (401 — เป็น OAuth token ไม่ใช่ API key ของ AI Studio)
  · OpenAI ยิงผ่านแล้ว แต่ **token accounting ของ OpenAI ยังไม่เคยวัดกับงานเต็มรอบ** (ping สั้นเท่านั้น)
...
- ⚠️ **`test_deployments::…newest_first_with_names` flaky ~1 ใน 5 รอบ** (ของเดิม ไม่ใช่ของรอบนี้)
  — `order_by(created_at)` ชนกันเพราะนาฬิกา Windows หยาบ · แก้จริงต้องมีคอลัมน์ลำดับ = migration
