...
0. 🎯 **รื้อ UI ตามเส้นทางงาน — เหลืออีก 3 ก้อน** (ก้อนที่ 1 เสร็จแล้ว 15 ส.ค.)
   - **ก้อน 2:** ตัวช่วย 3 หน้าจอตอนสร้างโปรเจกต์ (ไอเดีย+ไฟล์ → โครงสร้าง → ตรวจแผน)
     + เลือกทีมจาก `list_teams()` แทนพิมพ์ path เอง
   - **ก้อน 3:** แตกงานเป็นงานเบื้องหลัง (ลอก `services/runs.py`) + ปุ่ม commit แรก
     (`scaffold.git_commit_initial()` มีแล้วแต่ไม่มีปุ่ม) + ปุ่ม deploy ในเส้นทางของโปรเจกต์
