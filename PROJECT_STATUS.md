...
1. **ตั้งลำดับสำรองให้ใช้งานจริง** — ของที่ทำเสร็จยังไม่ถูกเปิดใช้ (`LLM_FALLBACKS` ยังว่าง =
   เจ้าหลักล่มก็ยังหยุดเหมือนเดิม) · แนะนำ `anthropic → openai → google` เพราะยิงผ่านครบทั้ง 3 แล้ว
