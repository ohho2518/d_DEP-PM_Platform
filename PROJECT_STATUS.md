...
> อัปเดตล่าสุด: **2026-08-28** | สถานะโดยรวม: **pytest 260/260 · ruff clean · build ผ่าน**
> — ฟอร์มเปิดโปรเจกต์ใหม่กลับมาทำงานเหมือน `new-project-studio` (เลือกทีม · แนบไฟล์ดีไซน์ ·
> ปุ่ม commit · ชนิดงานที่เลือกมีผลจริง) **และปิดบั๊กที่ทำให้ scaffold เขียนทับพื้นที่งานได้**
>
> 🔴 **แก้ backend แล้วต้องรีสตาร์ตทุกครั้ง** — ตัวที่รันอยู่ไม่มี `--reload` · อาการเวลาลืม:
> endpoint ใหม่ตอบ 404/422 ทั้งที่โค้ดถูก (route ที่เป็นคำตายตัวจะถูกจับเป็น `{project_id}`)
>
> อัปเดตก่อนหน้า: 2026-08-15 | **ใบสั่งงาน 2026-08-06 ปิดครบทุกข้อ · pytest 214/214**
...
### 🔴 ปิดบั๊ก scaffold เขียนทับพื้นที่งาน (2026-08-28) — ของหายจริงมาแล้ว 2 ครั้ง

**พบระหว่างจัดโฟลเดอร์ให้ `d_Songtify`** — โปรเจกต์บนบอร์ดตัวหนึ่งมี `local_path = D:\Dev_Proj`
คือ**รากของพื้นที่งานทั้งก้อน** ไล่ดูแล้วพบว่า `AGENTS.md` · `PROJECT_STATUS.md` ·
`CHANGELOG.md` · `docs/` ที่รากถูกแม่แบบ kit ทับไปเมื่อ 21:27 ของวันนั้น

- **ต้นเหตุ:** `resolve_target()` เขียนว่า `if root not in p.parents and p != root` —
  ท่อนหลังแปลว่า **"รากพอดีก็ผ่าน"** · ฟอร์มเก่ามีค่าปริยาย `D:\Dev_Proj\` ในช่อง target
  ⇒ กดสร้างโดยไม่แก้ = ยิงลงราก · **เกิดมาแล้ว 2 ครั้ง** (26 ส.ค. `d_ShopeeFeed` ทิ้งร่องรอยไว้
  ที่ `docs/API.md` หัวเรื่อง · 28 ส.ค. `d_Songtify`)
- **กู้ไม่ได้:** repo ที่รากไม่มี commit สักอัน · ไม่มี shadow copy บน D: · `d_BACKUP` ไม่มีสำเนา
  — แต่ของที่หายรอบล่าสุด**คือซากของอุบัติเหตุครั้งก่อน** ไม่ใช่เอกสารที่คนเขียนเอง
  · `_CANON` และทุกโฟลเดอร์ทีม/โปรเจกต์ **ไม่ถูกแตะเลย**
- **ปิด 3 ชั้น:** ปฏิเสธ target = ราก (400) · `backup_before_overwrite()` สำรองก่อนทับเสมอ
  (Rule 1) · UI ประกอบ path ให้เอง ไม่มีช่องที่ค่าปริยายเป็นราก — กติกาใหม่ **§9.1.17**
- 🐛 **บั๊กแถมที่เจอจากเทสต์ใหม่:** เรียก git ด้วย `text=True` เปล่า ๆ → Windows ถอดรหัสด้วย
  cp874 แล้วพังเมื่อ path มีตัวอักษรไทย (WORKING_RULES §6.1 ข้อเดียวกัน) · บังคับ UTF-8 แล้ว
- **เก็บกวาดแล้ว:** รากสะอาด (สำรองไว้ที่ `D:\Dev_Proj\BackUp\RootScaffoldSpill_20260828_224846\`)
  · `d_Songtify` รวมเป็นโฟลเดอร์เดียวที่ `4_RND\d_Songtify` (โค้ด Tauri เดิม + เอกสาร kit + git)
  · โปรเจกต์ผีที่ชี้ไปที่ราก **ปลด `local_path` เป็น null แล้ว** (ยังอยู่บนบอร์ด ลบได้ถ้าต้องการ)
- pytest 257 → **260** · ruff clean

### ฟอร์มเปิดโปรเจกต์ใหม่กลับมาเหมือน `new-project-studio` (2026-08-28)

**ที่มา:** ผู้ใช้ทักว่า "การทำงานไม่เหมือนเดิม" พร้อมชี้ `D:\Dev_Proj\4_RND\new-project-studio`
· เทียบโค้ดแล้ว **`services/scaffold.py` ยกมาครบทุกบรรทัดจริง** แต่**ฟอร์มบนหน้าเว็บยกมาไม่ครบ**
— backend รับ `team`/`dual_ps` มาตั้งแต่ ADR-05 แต่หน้าเว็บไม่เคยส่งให้เลย

| ของเดิมใน studio | ก่อนหน้านี้ | ตอนนี้ |
|---|---|---|
| เลือก **ทีมเจ้าของ** แล้วประกอบ path ให้ | ❌ พิมพ์ path เต็มเอง | ✅ `GET /scaffold-options` + auto-compose (ปริยาย `_INBOX`) |
| ช่อง **สังกัดทีมใน ecosystem** (`eco-team`) | ❌ | ✅ |
| ติ๊ก **เป็น Product/Service ด้วย** | ❌ | ✅ (เฉพาะ `eco-team`/`middleware`) |
| **ลากไฟล์ดีไซน์มาแนบ** ตอนสร้าง | ❌ (ทำได้เฉพาะบนบอร์ดทีหลัง) | ✅ เก็บลง `_design_input/` ให้ทันที |
| ปุ่ม **Commit เลย** | ❌ (มีแต่ฟังก์ชัน ไม่มีปุ่ม) | ✅ `POST /:id/commit` |
| (ของใหม่ที่นี่) **ชนิดงานที่เลือก** | ❌ ได้ `code` เสมอ | ✅ `/bootstrap` รับ `kind` — `doc` ได้เส้นทางของงานเอกสารจริง |

- **ราก `D:\Dev_Proj` เลิก hardcode ฝั่ง frontend** — มาจาก `SCAFFOLD_ALLOWED_ROOT` ที่เดียว
  (ก่อนหน้านี้ถ้าเปลี่ยนราก ฟอร์มจะโกหกทันที) · `scaffold._allowed_root()` → `allowed_root()` (public)
- ⚠️ **route ที่ขึ้นต้นด้วยคำตายตัวต้องประกาศก่อน `/{project_id}`** — ยืนยันด้วยของจริง:
  ยิง `/scaffold-options` ใส่ uvicorn ตัวเก่าได้ **422** เพราะถูกจับเป็น project_id
- **ตั้งใจไม่ยกกลับมา:** `ai_fill` (เรียก `claude -p` CLI — ขัด §9.1.13) · ปุ่มปิด server หลัง commit
  · terminal สตรีมสด (ข้อมูลชุดเดียวกันยังอยู่ครบใน `steps[]`)
- **`kind` ไปถึง `/bootstrap` แล้ว** (ปิดช่องที่ค้างไว้ตอนเช้า) — `code` ปริยายเหมือนเดิมทุกประการ ·
  `doc` ได้เส้นทางของงานเอกสาร (ข้ามขั้นโครงสร้าง) · **`idea` = 422 ตั้งแต่ชั้น validate**
  ยังไม่แตะดิสก์ เพราะไอเดียที่ยังไม่ลงมือไม่ควรมีโฟลเดอร์จริง (ทางของมันคือ `/promote`)
- pytest 250 → **257** · ruff clean · `npm run build` ผ่าน

### เส้นทางงาน 6 ขั้น + ชนิดงาน + ไอเดียขึ้นบอร์ด (2026-08-15) — ADR-06 · ก้อนที่ 1 ของการรื้อ UI

**ที่มา:** ผู้ใช้บอกว่า "ขั้นตอนสร้างโปรเจกต์ยังไม่ลื่น · UI เข้าใจยาก" แล้วขยายว่าต้องเห็นทั้งเส้น
ตั้งแต่ไอเดีย → โครงสร้าง → จ่ายงาน → รัน agent → deploy → **การตลาด** ·
วินิจฉัยจากโค้ดจริงได้ 7 จุด (ดูข้อเสนอที่ส่งไปแล้ว) — ต้นเหตุร่วมคือ **UI จัดตามของในระบบ
ไม่ได้จัดตามเส้นทางที่งานเดิน**

- **แถบ 6 ขั้นพร้อมสีประจำขั้น + "ขั้นต่อไป: …"** อยู่ทั้งบนบอร์ดและหน้ารวม
- 🔴 **ขั้นคำนวณสด ไม่เก็บใน DB** (`services/stages.py`) — อ่านจากโฟลเดอร์/task/deployment จริง
  ⇒ ไม่มีสถานะให้ใครกดให้ตรง จึงโกหกไม่ได้ · **`projects.kind` เก็บจริง** เพราะเป็นเจตนาของคน
- **3 ชนิดงาน:** `code` (ครบ 6 ขั้น) · `doc` (ข้ามโครงสร้าง · ขั้น 5 = "ส่งมอบ") ·
  `idea` (3 ขั้น จบที่ปุ่มยกระดับ) — migration `b6e2f4a81c39`, apply กับ DB จริงแล้ว
- **ไอเดียเก่า 9 เรื่องขึ้นบอร์ดแล้ว** จาก `IDEAs\` + `6_KM\Ideas\` (มติผู้ใช้: "ของเก่าจะดึงเข้าบอร์ด")
  · ไม่แตะไฟล์ต้นทาง · ยิงซ้ำได้ · `.md`+`.html` ชื่อเดียวกัน = เรื่องเดียว
- **ยังไม่ทำในก้อนนี้ (ตามแผน):** ตัวช่วย 3 หน้าจอตอนสร้างโปรเจกต์ (ก้อน 2) ·
  แตกงานเบื้องหลัง + ปุ่ม commit/deploy ในเส้นทาง (ก้อน 3) · **ขั้นการตลาดต่อ d_MOS** (ก้อน 4)
- pytest 214 → **250** · ruff clean · `npm run build` ผ่าน
...
- **pytest 260/260 ผ่าน · ruff clean · `npm run build` ผ่าน** (วัด 2026-08-28)
- **backend รีสตาร์ตแล้ว** (28 ส.ค. 22:48 · pid ใหม่) — `/scaffold-options` + `/:id/commit`
  ใช้งานได้จริง ยืนยันด้วย `openapi.json` 30 → 32 routes · ⚠️ แก้โค้ดหลังจากนั้นต้องรีสตาร์ตอีก
- 🧹 **`D:\Dev_Proj` (รากพื้นที่งาน) สะอาดแล้ว** — ซากจาก scaffold ถูกลบ สำรองไว้ที่
  `D:\Dev_Proj\BackUp\RootScaffoldSpill_20260828_224846\` (มี `CLAUDE.md`/`AGENTS.md` เดิมด้วย
  ถ้าจะเอากลับคืนก๊อบจากตรงนั้นได้) · ตอนนี้ที่รากไม่มีไฟล์ instruction ระดับพื้นที่งานแล้ว
...
- DB จริง migrate ถึง head **`b6e2f4a81c39`**
- **บอร์ดมี 13 โปรเจกต์** = งานมีโค้ด 4 (`a07f1fb2` · `7ffa2d4f` · `4d8004ff` งานจากเลขาที่ปิดแล้ว
  + `d_ACC` 17 backlog) · **ไอเดีย 9** ที่เพิ่งดึงขึ้นมาจากดิสก์
...
0. 🎯 **รื้อ UI ตามเส้นทางงาน — เหลืออีก 3 ก้อน** (ก้อนที่ 1 เสร็จแล้ว 15 ส.ค.)
   - **ก้อน 2:** ตัวช่วย 3 หน้าจอตอนสร้างโปรเจกต์ (ไอเดีย+ไฟล์ → โครงสร้าง → ตรวจแผน)
     · ✅ ~~เลือกทีมจาก `list_teams()` แทนพิมพ์ path เอง~~ · ✅ ~~ส่ง `kind` ไปกับ `/bootstrap`~~
     **ทำแล้วทั้งคู่ 28 ส.ค.**
   - **ก้อน 3:** แตกงานเป็นงานเบื้องหลัง (ลอก `services/runs.py`) + ปุ่ม deploy ในเส้นทางของโปรเจกต์
     · ✅ ~~ปุ่ม commit แรก~~ **ทำแล้ว 28 ส.ค.** (`POST /api/projects/:id/commit`)
   - **ก้อน 4:** ขั้นการตลาด → ต่อ **d_MOS** (`POST /api/mos/content` เข้าเป็น DRAFT เสมอ ·
     สนาม `used_facts` เสียบกับกติกาห้ามกุหลักฐานของเราได้ตรง) — **ต้องขอ token + กติกา
     จาก session ของ d_MOS ก่อน** (ห้ามแก้ข้ามรีโป)
5.1 **เศษจากงาน 28 ส.ค. (เล็ก ๆ รอเจ้าของเคาะ)** — โปรเจกต์ผี `d_Songtify ` (มีเว้นวรรคท้าย)
   ยังอยู่บนบอร์ด ปลด `local_path` แล้วจึงไม่อันตราย ลบได้ด้วย `DELETE /api/projects/:id`
   · โค้ด Tauri ใน `4_RND\d_Songtify` ยังไม่ commit — กดปุ่ม "Commit เลย" บนบอร์ดได้
   · `D:\Dev_Proj\prompts` อาจเป็นซาก scaffold รุ่นเก่า (ยังไม่ได้ตรวจลึก — kit มี `prompts/` ด้วย)
...
5. **รากพื้นที่งาน `D:\Dev_Proj` ตอนนี้ไม่มีไฟล์ instruction แล้ว** (ลบซาก scaffold ตามที่สั่ง
   28 ส.ค.) — จะให้มี `CLAUDE.md`/`AGENTS.md` ระดับพื้นที่งานไว้ไหม? ของเดิมอยู่ที่
   `D:\Dev_Proj\BackUp\RootScaffoldSpill_20260828_224846\` (เนื้อในเป็นแม่แบบ kit ไม่ใช่ของที่คนเขียน)
   · ถ้าอยากได้จริง ควร**เขียนใหม่ให้ตรงกับพื้นที่งาน** ไม่ใช่ก๊อบแม่แบบโปรเจกต์กลับมา
