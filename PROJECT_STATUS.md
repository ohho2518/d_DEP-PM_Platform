...
> อัปเดตล่าสุด: **2026-08-28** | สถานะโดยรวม: **pytest 257/257 · ruff clean · build ผ่าน**
> — ฟอร์มเปิดโปรเจกต์ใหม่กลับมาทำงานเหมือน `new-project-studio` (เลือกทีม · แนบไฟล์ดีไซน์ ·
> ปุ่ม commit · ชนิดงานที่เลือกมีผลจริง) · 🔴 **ต้องรีสตาร์ต backend ก่อนใช้** — ตัวที่รันค้างอยู่ไม่มี `--reload`
> จึงยังไม่รู้จัก endpoint ใหม่ (`/scaffold-options` ตอบ 422 เพราะถูกจับเป็น project_id)
>
...
- **pytest 257/257 ผ่าน · ruff clean · `npm run build` ผ่าน** (วัด 2026-08-28)
- 🔴 **backend ที่รันค้างอยู่ยังเป็นโค้ดเก่า** (ไม่มี `--reload`) — ฟอร์มเปิดโปรเจกต์ใหม่
  จะยังไม่มีช่องเลือกทีมจนกว่าจะ **หยุด/เปิดระบบใหม่** (`Start-DEP-PM.exe` → [S] แล้วเปิดใหม่)
