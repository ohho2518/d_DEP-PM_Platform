...
- 🔴 **งานที่ต้องรอข้อมูลจากคน ห้ามเป็น `depends_on` ของงานส่งมอบ** — แยกมันเป็น task ของตัวเอง
  แล้วให้งานรวมเล่ม/ส่งมอบ **ผลิตฉบับเต็มได้เลยโดยเว้นช่องว่างติดป้าย
  `[รอข้อมูล: <อะไร> จาก <ใคร>]`** ตรงจุดที่ยังไม่มีข้อมูล แล้วให้คนเติมทีหลัง
  · เจอจริง 2026-08-14: ตัวเลขสถิติชิ้นเดียวที่ไม่มี ทำให้คู่มือที่เขียนได้แล้ว 80%
  **ไม่ถูกผลิตเลยแม้แต่ฉบับร่าง** เพราะ task "รวมเล่ม" ไปผูกกับ task ที่ตัน
  · หลักคิด: ผูก dependency เฉพาะกับงานที่ **ทีมทำเองจนจบได้** — เอกสารที่มีช่องว่างติดป้าย
  มีค่ากับคนอ่านมากกว่าเอกสารที่ไม่มีอยู่จริง
...
- 🔵 **ขาดข้อมูล "บางจุด" ≠ ทำงานไม่ได้** — ถ้าผลิตของหลักได้แล้วและเว้นเฉพาะจุดที่ขาดไว้
  โดยติดป้าย `[รอข้อมูล: …]` หรือ "ต้องการข้อมูลจากคน:" ตรงจุดนั้น → **approve** แล้วให้คน
  มาเติมทีหลัง · **ห้ามยกทั้งชิ้นให้คนเพราะรายละเอียดจุดเดียว**
  · เจอจริง 2026-08-14: คู่มือที่เขียนครบทุกหัวข้อถูกตีเป็น `needs_human` เพราะขาดข้อมูล
    "ปุ่มอยู่ตรงไหนบนจอ" อย่างเดียว ⇒ ผู้ใช้ไม่ได้เอกสารเลยทั้งที่ 95% พร้อมส่ง
  · เกณฑ์ตัดสิน: **ถ้าตัดส่วนที่ขาดออกแล้วผลงานยังมีประโยชน์กับคนอ่าน = approve**
- 🚩 **งานที่ติดทั้งชิ้นเพราะไม่มีข้อมูล/สิทธิ์ที่ agent หามาเองไม่ได้** (ทำของหลักไม่ได้เลย —
  work product เป็นรายงานว่าทำไม่ได้ พร้อมหัวข้อ "ต้องการข้อมูลจากคน:")
  → ตอบ `"approved": false` **และ `"needs_human": true`**
