...
**พอร์ต — ทะเบียนกลางอยู่ที่ canon ห้ามก๊อบตารางมาไว้ที่นี่:**

📌 **`D:\Dev_Proj_KM\d_InnoHub\_CANON\SERVICE_PORTS.md`** — ทะเบียนพอร์ตของทั้ง eco
+ กติกา 3 ข้อก่อนจองพอร์ต + เกณฑ์ bind (อ่านจาก path นั้นตรง ๆ)

ที่ต้องรู้สำหรับรีโปนี้: **DEP-PM backend = `8500`** · frontend = `3000` ·
`8000` d_CEO และ `8400` d_Jarvis web **รันค้างตลอด ห้ามหยุด**

> ⚠️ ตารางพอร์ตเคยถูกก๊อบไว้ 3 ชุดในรีโปนี้ (ใบสั่งงาน 3 ส.ค. Ticket 3.5) — เลิกแล้ว
> เพราะสำเนาคือจุดเริ่มของข้อมูลขัดกันเอง · เหตุที่ทะเบียนกลางเกิดขึ้นคือรีโปนี้เองเคยจอง
> `8400` ทับ d_Jarvis web เมื่อ 2026-08-02
...
LLM_BUDGET_USD=        # เพดานค่าใช้จ่าย **ต่อโปรเจกต์** (USD) · 0 = ไม่จำกัด (ปริยาย)
LLM_BUDGET_ACTION=     # เกินเพดานแล้ว: warn (ปริยาย) | stop = ไม่เริ่ม task ใหม่
                       # ↑ แก้จากหน้า /settings ได้เช่นกัน
LLM_PRICE_<เจ้า>_IN/OUT=  # ราคาต่อ 1 ล้านโทเคน ที่ใช้ **ประมาณการ** ค่าใช้จ่าย
                       # (ANTHROPIC/OPENAI/GOOGLE) — ยังไม่ยืนยันกับบิลจริง ห้ามอ้างเป็นตัวเลขจริง
...
16. **ตัวเลขเงินในระบบเป็น "ประมาณการ" เสมอ ห้ามเรียกว่าค่าใช้จ่ายจริง** — คิดจากราคาที่ตั้งไว้
    ...
    **ห้ามตัดกลาง task** ที่จ่ายค่าโทเคนไปแล้ว (เจตนาเดียวกับปุ่มยกเลิกรอบรัน)
17. **scaffold ห้ามเขียนลง `SCAFFOLD_ALLOWED_ROOT` ตรง ๆ และห้ามทับไฟล์โดยไม่สำรอง** —
    `resolve_target()` ปฏิเสธ target ที่เท่ากับรากพอดี (`p == root` = **400** ไม่ใช่ผ่าน) ·
    `backup_before_overwrite()` ต้องรันก่อนการเขียนครั้งแรกของทุกรอบ (WORKING_RULES Rule 1)
    · **เกิดขึ้นจริง 2 ครั้ง** (2026-08-26 `d_ShopeeFeed` · 2026-08-28 `d_Songtify`):
    เอกสารของ `D:\Dev_Proj` ทั้งก้อนถูกแม่แบบทับ กู้ไม่ได้เพราะ repo ที่รากไม่มี commit
    · เพิ่มไฟล์ใหม่ที่ scaffold เขียน = เพิ่มใน `_OVERWRITTEN_EXTRAS` ด้วย ไม่งั้นสำรองไม่ครบ
    · เรียก git ต้องใส่ `**_GIT_TEXT` (UTF-8) เสมอ — `text=True` เปล่า ๆ ถอดรหัสด้วย ANSI
    ของเครื่อง (cp874) แล้วพังทันทีที่ path มีตัวอักษรไทย
