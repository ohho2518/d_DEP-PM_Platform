...
### วิธีปกติ — ดับเบิลคลิก **"เปิดระบบ DEP-PM"** บนหน้า Desktop

(ชอร์ตคัตชี้ไปที่ `Start-DEP-PM.exe` ที่รากของรีโป · สร้างใหม่/ซ่อมได้ด้วย `.\tools\launcher\make-shortcut.ps1`)

ตรวจของที่ต้องมี → อัปเกรดฐานข้อมูล (สำรองก่อนเสมอ) → สตาร์ต `:8500` + `:3000` → เปิดเบราว์เซอร์
· วัดจริง **7.8 วินาที** จากที่ยังไม่มีอะไรรัน · ปิดหน้าต่างเฉย ๆ ระบบยังรันต่อ · เปิดใหม่กด **[S]** เพื่อหยุด
· **ไม่แตะพอร์ต 8000/8400** และ**ไม่สตาร์ตซ้อน**ถ้ามีตัวที่รันอยู่แล้ว
· ยังไม่มีไฟล์ `.exe`? สร้างเองด้วย `.\tools\launcher\build.ps1` (ดู `tools/launcher/README.md`)

### วิธีมือ (dev — เวลาต้องการ log สด หรือแก้โค้ดแล้วอยากได้ `--reload`)
...
## 4.2 AI เจ้าหลักล่ม / เครดิตหมด — ทำยังไง (ใบสั่งงาน 2026-08-06)

**อาการ:** รอบรันจบเป็น `failed` โดย `error` ขึ้นต้นว่า *"ผู้ให้บริการ LLM ใช้ไม่ได้ทั้งหมด"* ·
task ที่ค้างอยู่กลายเป็น `escalated` (ไม่ค้าง `in_progress`) · `GET /health` → `llm_providers` หายไปหนึ่งเจ้า

**แก้ที่หน้าเว็บ ไม่ต้อง restart:**

1. เปิด **`/settings`** (เมนู "⚙ ตั้งค่า AI") → กด **ทดสอบทั้งหมด** เพื่อดูว่าเจ้าไหนยังไหว
   - ❌ *บัญชีใช้ไม่ได้* = เครดิตหมด/คีย์ผิด → เติมเครดิตหรือใส่คีย์ใหม่
   - ⏳ *ขัดข้องชั่วคราว* = รอสักครู่แล้วทดสอบใหม่ (ระบบลองซ้ำให้เองอยู่แล้ว)
   - ⚠️ *คำขอไม่ถูกต้อง* = ชื่อรุ่นผิด (ไม่ใช่ปัญหาบัญชี — **ระบบจะไม่สลับเจ้าให้** โดยตั้งใจ)
2. ตั้ง **ตัวหลัก** เป็นเจ้าที่ยังใช้ได้ + ติ๊ก **ตัวสำรอง** ไว้อย่างน้อยหนึ่งเจ้า → **บันทึก**
   (มีผลทันที · ค่าเดิมถูกสำรองไว้ที่ `BackUp/EnvSettings_<เวลา>/.env`)
3. งานที่ค้าง: บนบอร์ดกด **ตีกลับเข้าคิว** (`escalated → planned`) แล้วกด **Run** ใหม่
...
## 4.3 คุมค่าใช้จ่าย (§5 ใบสั่งงาน 2026-08-06)

**ดูว่าโปรเจกต์หนึ่งใช้ไปเท่าไร:** แถบ `💰 ใช้ไปประมาณ $x` บนหน้าบอร์ด (ยอดแยกรายเจ้าอยู่ในแถบเดียวกัน)
· ต่อ task ดูได้ในหน้ารายละเอียด · ทาง API: `GET /api/projects/:id/usage`

**ตั้งเพดาน:** `/settings` → *เพดานค่าใช้จ่าย (ต่อโปรเจกต์)* → ใส่ตัวเลข USD (0 = ไม่จำกัด) แล้วเลือก

| โหมด | เกิดอะไรขึ้นเมื่อถึงเพดาน |
|---|---|
| **เตือนอย่างเดียว** (ปริยาย) | ตัวเลขบนบอร์ดขึ้นสีแดง งานเดินต่อตามปกติ |
| **หยุดไม่เริ่มงานใหม่** | รอบรัน**ไม่หยิบ task ถัดไป** จบเป็น `succeeded` พร้อมเหตุ `💰 …` บนบอร์ด · **task ที่กำลังทำอยู่ยังทำจนจบ** · งานที่เหลือค้าง `planned` — ขยับเพดานแล้วกด **Run** ต่อได้ทันที |

> 🔴 **ตัวเลขเป็น "ประมาณการ" ไม่ใช่บิลจริง** — คิดจากราคาที่ตั้งไว้ใน `.env` (`LLM_PRICE_*`,
> ค่าปริยายจากหน้าประกาศราคา ณ 14 ส.ค. 2026 **ยังไม่เทียบกับบิลของบัญชีนี้**) ส่วนลด/เครดิตไม่ถูกนับ
> · ถ้าเห็นวงเล็บ *"ไม่รวมโทเคนของงานเก่าที่ระบุเจ้าไม่ได้"* แปลว่า**ของจริงสูงกว่าที่แสดง**
> (งานก่อน 14 ส.ค. ไม่ได้บันทึกว่าเจ้าไหนใช้) · ราคาแก้ที่ `.env` เท่านั้น แล้ว restart
> — จงใจไม่ให้แก้จากหน้าเว็บ เพราะเป็นตัวเลขที่ต้องยืนยันกับบิลก่อน
...
- ✅ **วัดกับโปรเจกต์ที่ปนกัน** (`17db2a67`, 4 tasks — คู่มือที่ข้อมูลครบ + หัวข้อสถิติที่ระบบไม่มี):
  งานที่ทำได้ → `done` rev 0 (`needs_human=false`) · งานที่ต้องรอคน → `escalated` rev 0
  (`needs_human=true`) ⇒ **reviewer ไม่ได้ใช้ verdict ใหม่พร่ำเพรื่อ** (ยังเป็นตัวอย่างเดียว)
- 📌 **สิ่งที่รอบนี้เปิดต่อ:** PM ผูกงาน "รวมเล่ม" ไว้กับหัวข้อที่ติด ⇒ ตัวเลขหายชิ้นเดียว
  ทำให้เอกสารที่เขียนได้ 80% ไม่ถูกผลิตเลย — ยังไม่แก้ (ดู PROJECT_STATUS Next Tasks #2)
