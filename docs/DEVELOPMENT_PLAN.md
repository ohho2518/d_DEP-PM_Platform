...
### ADR-05: รวม `new-project-studio` เข้ามาเป็นความสามารถ "เปิดโปรเจกต์ใหม่จริง" (2026-08-14)

**บริบท:** `D:\Dev_Proj\4_RND\new-project-studio` เป็นเครื่องมือ local (FastAPI `:8848`) ที่ bootstrap
โฟลเดอร์โปรเจกต์ใหม่ใต้ `D:\Dev_Proj` — ทำงาน 2 ชั้น: **scaffold** (Python ตายตัว: copy kit · git init ·
`.gitignore` · `requirements.txt`) และ **ai_fill** (เรียก `claude -p --agent new-project-bootstrapper`
เติมเอกสารจากไฟล์ดีไซน์) · มี `pm_sync.py` ที่ยิง `POST /api/projects` มาลงทะเบียนในบอร์ดเราอยู่แล้ว
(แต่ docstring ชี้พอร์ต `8000` ซึ่งเป็นของ d_CEO — ผิดมาตั้งแต่ต้น)

- **ตัดสินใจ (ผู้ใช้เลือก 2026-08-14):** ยก**เฉพาะส่วน deterministic** เข้ามาเป็นของ DEP-PM ·
  ส่วน "เติมเอกสารด้วย AI" **ไม่ยกโค้ดมา** แต่ให้กลายเป็น **งานบนบอร์ด** ที่ orchestrator เดิมทำ
- **เหตุผล:**
  1. `scaffold` ไม่มี AI เลย ย้ายมาแล้วใช้ได้ทันที ไม่ชนกติกาข้อไหน
  2. `ai_fill` เรียก **CLI ของ Claude Code** ซึ่ง**ขัด §9.1.13** (ทุก LLM call ต้องผ่าน
     `agents/providers.py`) — ยกมาทั้งดุ้น = ต้องเจาะข้อยกเว้นถาวรให้มี LLM path ที่สองในระบบ
  3. งาน "อ่านไฟล์ดีไซน์แล้วเขียนเอกสาร" คือสิ่งที่บอร์ดนี้ทำได้ดีกว่าอยู่แล้ว: ได้กติกา**ห้ามกุหลักฐาน**
     · `needs_human` · reviewer · audit · **โทเคนแยกตามเจ้า** · **ลำดับสำรองเมื่อเจ้าหลักล่ม** —
     ของพวกนี้ CLI headless ไม่มีสักอย่าง
  4. เหลือ service เดียว (`:8500`) ไม่ต้องจำว่ามีตัวที่ `:8848` อีกตัว
- **สิ่งที่ยอมแลก:** ปุ่มเดียวจบแบบ headless หายไป — การเติมเอกสารกลายเป็น "กด Run แล้วดูบอร์ด"
  ช้ากว่าเดิมแต่**ตรวจสอบย้อนหลังได้และมีคนตรวจ** ซึ่งตรงกับเหตุผลที่ระบบนี้มีอยู่
- **ทางถอย:** รีโป `new-project-studio` เก็บไว้อ่านอย่างเดียว (หยุดพัฒนา) — ถ้าวันไหนต้องการโหมด
  headless จริง ๆ ยังรันของเดิมได้ ไม่ได้ลบทิ้ง

### ADR-06: เส้นทางงาน 6 ขั้น — ชนิดงานเก็บใน DB แต่ **ขั้นคำนวณสด** (2026-08-15)

**บริบท:** ผู้ใช้บอกว่า "UI ยังเข้าใจยาก" — สาเหตุที่วัดได้คือหน้าจอถูกจัดตาม**ของในระบบ**
(Portfolio · New Project · Board · Deployments · Settings) ไม่ได้จัดตาม**เส้นทางที่งานเดินจริง**
(ไอเดีย → โครงสร้าง → แผนงาน → ลงมือ → ส่งขึ้นระบบ → การตลาด) และไม่มีที่ไหนบอกว่า
"ตอนนี้อยู่ตรงไหน ต่อไปทำอะไร"

- **ตัดสินใจ:** เพิ่ม `projects.kind` (`code`/`doc`/`idea`) เป็นคอลัมน์จริง ·
  ส่วน **ขั้น (stage) ไม่เก็บ** — คำนวณสดที่ `services/stages.py` ทุกครั้งที่อ่าน
- **เหตุผลที่ไม่เก็บขั้น:** สถานะที่ต้องมีคน (หรือโค้ด) มาอัปเดตให้ตรง จะไม่ตรงในที่สุด —
  บทเรียนตรงนี้เสียไปแล้วครั้งหนึ่งเมื่อ 3 ส.ค. (รายงานว่า "เสร็จ" ทั้งที่ไม่มีชิ้นงาน · QC จับได้)
  ⇒ อ่านจากหลักฐานที่ปลอมไม่ได้แทน: มีโฟลเดอร์จริงไหม · task พ้น `backlog` แล้วหรือยัง ·
  เหลืองานค้างกี่ใบ · เคย deploy สำเร็จหรือยัง
- **เหตุผลที่ `kind` ต้องเก็บ:** เป็น**เจตนาของคน** — งานเอกสารกับงานมีโค้ดหน้าตาข้อมูลเหมือนกัน
  ระบบเดาไม่ได้ว่าเจ้าของตั้งใจให้จบที่ deploy หรือจบที่ส่งมอบไฟล์
- **ไอเดียเป็นชนิดงาน ไม่ใช่สถานะ:** เก็บโจทย์ไว้ก่อน ให้ agent ช่วยศึกษาได้ แล้วค่อย
  **ยกระดับ** (`POST /:id/promote`) เป็น `code`/`doc` โดย**งานที่ศึกษาไว้อยู่ครบ** ·
  ของเก่าที่กองอยู่ในดิสก์ถูกดึงขึ้นบอร์ดด้วย (มติผู้ใช้ 2026-08-15) — `services/ideas.py`
- **สิ่งที่ยอมแลก:** คำนวณทุกครั้งที่อ่าน = query เพิ่มต่อโปรเจกต์ (2 query: task counts + deployment)
  · ยอมรับได้ที่ขนาดปัจจุบัน (สิบกว่าโปรเจกต์) · ถ้าวันหนึ่งช้า ค่อย cache ที่ชั้นบน
  **ไม่ใช่เปลี่ยนไปเก็บสถานะ**
- **ทางถอย:** `kind` มี `server_default='code'` ⇒ ของเดิมทั้งหมดหมายความเหมือนเดิมเป๊ะ ·
  ถอย migration ตัวเดียวจบ
...
| **S2 — UI** | หน้า `/projects/new` มีโหมด "สร้างโฟลเดอร์โปรเจกต์จริงด้วย" (target · purpose · stack) แล้วพาไปบอร์ด | `frontend/src/app/projects/new/` · `lib/{types,api}.ts` |
| **S3 — เติมเอกสารเป็นงานบนบอร์ด** | อัปโหลดไฟล์ดีไซน์ → เก็บที่ `docs/_design/` ของโปรเจกต์ → PM แตกงาน → agent เขียนเอกสาร → **ขั้นเขียนผลงานลงไฟล์จริง** | `services/deliverable_writer.py` (ใหม่) + endpoint อัปโหลด |

**ข้อจำกัดที่ต้องออกแบบเผื่อใน S3:** agent ของเราผลิต**ข้อความ** ไม่ได้เขียนไฟล์เอง (providers เป็น
text-only ตาม §9.1.13) ⇒ ต้องมีขั้น deterministic ที่เอา work product ของ task ไปเขียนลงไฟล์
· **guard บังคับ:** เขียนได้เฉพาะใต้โฟลเดอร์ของโปรเจกต์นั้น · **สำรองไฟล์เดิมก่อนทับเสมอ**
(WORKING_RULES Rule 1) · ไม่ auto-commit — คง stage ให้คนตรวจเหมือนที่ studio ทำ

**กติกาที่ต้องเพิ่มตอน S1:**
- `SCAFFOLD_ALLOWED_ROOT` (ปริยาย `D:\Dev_Proj`) — target นอกรากนี้ = 400 (กัน scaffold ลงที่มั่ว)
- **แม่แบบเอกสาร (kit) ก๊อบเข้ามาอยู่ในรีโปนี้** เพราะ studio จะหยุดพัฒนา — เจ้าของแม่แบบย้ายมาที่นี่
  · ⚠️ **ไม่เกี่ยวกับ `_CANON`** ที่ห้ามคัดลอก — kit เป็นแม่แบบของเครื่องมือ ไม่ใช่ต้นฉบับความจริง
- `pm_sync.py` ของ studio เลิกใช้ (ตรรกะย้ายมาอยู่ใน endpoint ฝั่งเรา) — จบปัญหาพอร์ต `8000` ที่ผิด
