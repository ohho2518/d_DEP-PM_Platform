...
## ผลงาน (ตัวชิ้นงานจริง)

### ออกแบบ schema
<ผลงานฉบับล่าสุดของ task นั้นเต็ม ๆ>
...
> 🔴 **หัวข้อ "ผลงาน" ห้ามตัดทิ้ง** — QC ของ d_CEO ตรวจจาก **ตัวชิ้นงาน** ไม่ใช่สรุปสถานะ
> (2026-08-03 รายงานที่มีแต่สรุป task board ถูกตอบกลับ `rejected` ทันที พร้อมข้อเสนอเข้า SOP ของเขาว่า
> *"ผลงานที่ส่ง QC ต้องแนบตัวชิ้นงานจริง ไม่ใช่รายงานสรุปสถานะ task"*)
> · แนบ **ผลงานฉบับล่าสุดของทุก task ที่เสร็จ** · เพดาน 8,000 ตัวอักษร/task และ 40,000 ตัวอักษรรวม
> — ถ้าตัด ต้องเขียนบอกในรายงานว่าตัดของใครไป
...
|---|---|
| d_CEO ปิดอยู่ | `/api/ceo/*` ตอบ **503** พร้อมข้อความ (ยกเว้น `/status` ที่ตอบ `online:false`) · เส้นทางอัตโนมัติหลังรอบรันจบ บันทึกเหตุไว้ใน `ceo_report.detail` **ไม่ทำให้รอบรันเป็น `failed`** |
...
| รอบรันจบและโปรเจกต์มาจาก d_CEO | รายงานอัตโนมัติหลังงานจบ (อยู่ในเธรดเบื้องหลังตั้งแต่ Phase 2) — ผลอ่านได้ที่ `ceo_report` ของ `GET /api/projects/:id/run` **ไม่ใช่ response ของ `POST /run` ที่ตอบ 202 ไปก่อนแล้ว** |
