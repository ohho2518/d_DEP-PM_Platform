...
| งานยังไม่จบ | `report` ตอบ `ready:false` และ **ไม่แตะ d_CEO** |
...
- ✅ pull / report / guardrail / degrade เมื่อออฟไลน์ — ครอบด้วย 17 เทสต์ใน
  `backend/tests/test_ceo_integration.py` (stub client ผ่าน dependency override ไม่ mock HTTP)
- ⬜ **ยังไม่ได้ทดสอบกับงาน R&D จริง** — ต้องมีคนสั่งงานผ่าน Jarvis ให้ทีม R&D ก่อน
  (การสร้าง task ทดสอบใน d_CEO = เขียนข้อมูลจริงของผู้ใช้ ต้องขออนุญาตก่อน)
