...
- 🔒 **ปลายทางที่มีไฟล์เดิมอยู่ → สำรองก่อนทับเสมอ** ลง `<target>/BackUp/Scaffold_<เวลา>/`
  แล้วบอกไว้ใน `steps[]` (WORKING_RULES Rule 1) · โฟลเดอร์ใหม่เอี่ยมไม่สร้าง `BackUp/` เปล่า
- **400** เมื่อ target อยู่นอก `SCAFFOLD_ALLOWED_ROOT` หรือ `relation` ไม่รู้จัก
  — ตรวจก่อนแตะดิสก์ ไม่มีโฟลเดอร์/โปรเจกต์ค้าง
- 🔴 **400 เมื่อ target = ราก `SCAFFOLD_ALLOWED_ROOT` พอดี** — เปิดโปรเจกต์ทับพื้นที่งานไม่ได้
  (เคยหลุดมาแล้ว 2 ครั้ง เอกสารที่รากถูกแม่แบบทับ — ดู AGENTS.md §9.1.17)
