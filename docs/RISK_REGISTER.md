...
| 5 | 🟢 | ~~**OpenAI/Gemini executors ไม่เคยรันกับ service จริง**~~ | Team Mode อาจพังตอนใช้จริง | — | **ยิงจริงแล้ว 2026-08-14** ผ่านปุ่มทดสอบของหน้า `/settings`: **OpenAI `gpt-5.2` ใช้ได้จริง (3.4 วิ)** · Anthropic ใช้ได้ · **Gemini ตอบ 401 — คีย์ใน `.env` เป็น OAuth token ไม่ใช่ API key** (ต้องออกคีย์ใหม่จาก AI Studio) | **Mitigated (เหลือคีย์ Gemini)** |
...
| 5.2 | 🔴 | **หน้า `/settings` แก้คีย์ได้โดยไม่มี authentication** | ใครที่ยิงถึงพอร์ตได้ เปลี่ยน/ลบคีย์ของระบบได้ | ต่ำตราบใดที่ bind `127.0.0.1` | คีย์ไม่เคยออกทาง API แบบเต็ม (mask เท่านั้น) · สำรอง `.env` ทุกครั้งที่เขียน · **ทำให้ auth กลายเป็นบล็อกเกอร์ก่อน expose พอร์ต** (docs/SECURITY.md) | **Open (ยอมรับเฉพาะ localhost)** |
